...
import ApplicationDetailsModal from "../opportunity/ApplicationDetailsModal";
...
export default function MyCollection() {
  ...
  const Tag = ({ children }) => (
    <span className="inline-flex items-center rounded-md bg-slate-100 px-2 py-1 text-[15px] font-medium text-slate-700">
      {children}
    </span>
  );
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <div
              className={`grid h-10 w-10 place-content-center rounded-lg text-white ${brandColor}`}
            >
              <span className="text-base font-semibold">{avatarLetter}</span>
            </div>
            <div>
              <h3 className="text-[18px]  font-medium text-slate-900">{title}</h3>
              <p className="mt-0.5 text-[16px] text-slate-500">{org}</p>
            </div>
          </div>

          <button
            className="p-1.5 text-slate-500 hover:text-slate-700 focus:outline-none hover:cursor-pointer"
        ...
        <div className="mt-4 flex items-center gap-3 text-sm text-slate-600">
          <div className="flex items-center gap-1.5">
            <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M11 5V11L15 13M21 11C21 16.5228 16.5228 21 11 21C5.47715 21 1 16.5228 1 11C1 5.47715 5.47715 1 11 1C16.5228 1 21 5.47715 21 11Z" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
          ...
          <button
            onClick={() => {
              setSelectedItem({
                ...
                posted: "",
                deadline: null,
                description: `${title} • ${org} • ${location}`,
                images: [],
              });
              setModalOpen(true);
            }}
            className="w-full rounded-lg bg-[#0B1D5C] px-4 py-2.5 text-sm font-medium text-white hover:cursor-pointer hover:bg-[#0A2A6A] focus:outline-none focus:ring-2 focus:ring-slate-200"
            type="button"
          >
            {cta}
          </button>
  ...
  // Use singular for type matching
  const options = ["All", "Job", "Scholarship", "Lesson", "Question"];
  ...
  // Compose filters: category and search
  const filteredItems = filterByCategory(items, selectedType).filter(i =>
  ...
  // Modal state for Application Details
  ...
  return (
    <div className="min-h-screen bg-gray-100 text-slate-900">
            ...
            {/* logo  */}
              ...
              <div className="position-absolute w-full -mt-[60px] rounded-t-2xl shadow-2xs bg-white h-full">
                ...
                <ul className="flex flex-col gap-y-4 mt-18 ml-5">
                  <li className="p-2">
                    <span className="flex items-center gap-x-5 hover:cursor-pointer" onClick={() => { router.push("/homepage"); }}>
                      <svg
                        width="30"
                        height="30"
                        viewBox="0 0 20 19"
                  ...
                  <li className="p-2 hover:cursor-pointer">
                    <span className="flex items-center gap-x-5" onClick={() => { router.push("/mynetwork"); }}>
                      <svg
                        width="30"
                        height="30"
                        viewBox="0 0 20 19"
                  ...
                  <li className={`p-2 hover:cursor-pointer ${activeMenu === "Collection" ? "bg-blue-600 px-2 py-4 text-white rounded-[10px]" : ""}`}>
                    <span className="flex items-center gap-x-5" onClick={() => setActiveMenu("Collection")}>
                      <svg
                        width="30"
                        height="30"
                        viewBox="0 0 20 20"
      ...
      <div className="mx-auto w-[1425px] ml-105 bg-white h-[100px] p-2 sm:px-6 lg:px-8 py-8">
        <div className="ml-5">
          <h1 className="text-2xl -mt-[20px] sm:text-3xl font-semibold tracking-tight">My Collection</h1>
      ...
      {/* Controls */}
      <div className="mt-6 flex flex-col gap-3 w-[1330px] ml-115 mb-8 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <div className="relative w-full sm:w-80">
              ...
              <input
                ...
                className="w-full rounded-lg border-2 bg-white py-2.5 pl-10 pr-3 text-sm placeholder-slate-400  focus:border-slate-300 focus:outline-none focus:ring-2 focus:ring-slate-200"
                value={search}
            ...
            <button
              type="button"
              className="inline-flex items-center gap-2 rounded-lg border bg-white px-3 py-2 text-sm  focus:outline-none focus:ring-2 focus:ring-slate-200 hover:bg-blue-100 hover:cursor-pointer"
              onClick={() => setShowFilters(!showFilters)}
            >
              
              <svg width="14" height="10" viewBox="0 0 14 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M1 1H13M3 5H11M5 9H9" stroke="black" strokeWidth="1.5" strokeLinecap="round"/>
              </svg>
          ...
          <div className="text-[16px] text-slate-500 self-start sm:self-auto">
            {items.length} {items.length === 1 ? "item" : "items"}
          </div>
      </div>

      {showFilters && (
            <div className="mb-8 ml-116 p-4 rounded-2xl w-[58%] bg-white shadow-md">
              <h3 className="text-[16px] mb-5">Filter by type</h3>
                ...
                {options.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        "whitespace-nowrap rounded-lg border px-3 py-1.5 text-sm transition",
                        active
      ...
      <div className="mx-auto ml-107 w-full max-w-6xl px-4 sm:px-6 lg:px-8 pb-12">
